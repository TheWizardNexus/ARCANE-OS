...
try {
  ...
  assert.equal(clean.repairRequired, false);
  assert.equal(clean.disposition, 'current');
  assert.equal(clean.action, 'current');
  ...
  assert.equal(mismatchedVersion.blocked, false, 'an invalid identity requires repair; it is not a trusted downgrade comparison');
  assert.equal(mismatchedVersion.blockedReason, null);
  assert.equal(mismatchedVersion.repairRequired, true);
  assert.equal(mismatchedVersion.repairReason, 'identity-invalid');
  assert.equal(mismatchedVersion.disposition, 'repair-required');
  assert.equal(mismatchedVersion.action, 'repair');
  assert.match(mismatchedVersion.installedIdentity.reason, /versions do not match/i);
  await fs.writeFile(path.join(installRoot, 'arcane-install.json'), JSON.stringify(manifest, null, 2));

  const newerBundlePayload = Buffer.from(JSON.stringify({ name:'Arcane OS',version:'0.9.0' }, null, 2));
  const newerReleasePayload = Buffer.from(JSON.stringify({ name:'Arcane OS',version:'0.9.0' }, null, 2));
  const newerManifest = structuredClone(manifest);
  newerManifest.version = '0.9.0';
  newerManifest.integrity.files[0] = { path:'arcane-bundle.json',size:newerBundlePayload.length,sha256:digest(newerBundlePayload) };
  newerManifest.integrity.files[1] = { path:'arcane-release.json',size:newerReleasePayload.length,sha256:digest(newerReleasePayload) };
  await fs.writeFile(path.join(installRoot, 'arcane-bundle.json'), newerBundlePayload);
  await fs.writeFile(path.join(installRoot, 'arcane-release.json'), newerReleasePayload);
  await fs.writeFile(path.join(installRoot, 'arcane-install.json'), JSON.stringify(newerManifest, null, 2));
  const trustedNewer = await call('installation.status');
  assert.equal(trustedNewer.installedIntegrity.ok, true);
  assert.equal(trustedNewer.installedIdentity.ok, true);
  assert.equal(trustedNewer.blocked, true);
  assert.equal(trustedNewer.blockedReason, 'downgrade');
  assert.equal(trustedNewer.repairRequired, false);
  assert.equal(trustedNewer.disposition, 'downgrade-blocked');
  assert.equal(trustedNewer.action, 'blocked');
  await fs.writeFile(path.join(installRoot, 'arcane-bundle.json'), bundlePayload);
  await fs.writeFile(path.join(installRoot, 'arcane-release.json'), releasePayload);
  await fs.writeFile(path.join(installRoot, 'arcane-install.json'), JSON.stringify(manifest, null, 2));
  ...
  for (const legacyVersion of [version, '0.9.0']) {
    ...
    assert.equal(blockedLegacy.blocked, false);
    assert.equal(blockedLegacy.repairRequired, true);
    assert.equal(blockedLegacy.repairReason, 'identity-invalid');
    assert.equal(blockedLegacy.action, 'repair');
  }
  ...
  assert.equal(wrongLegacyProduct.blocked, false);
  assert.equal(wrongLegacyProduct.repairRequired, true);
  ...
  assert.equal(sameSizeTamper.blocked, false);
  assert.equal(sameSizeTamper.repairRequired, true);
  assert.equal(sameSizeTamper.repairReason, 'identity-invalid');
