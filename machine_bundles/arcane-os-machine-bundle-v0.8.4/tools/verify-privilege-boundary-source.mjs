...
const [guard, core, windows, linux, host, windowsBuild, targetBuild, releaseIntegrity, machineContent, targetFinalizer, windowsSecurity, windowsSigning] = await Promise.all([
  ...
  fs.readFile(path.join(root, 'tools/windows-signing.psm1'), 'utf8'),
]);
...
assert.match(windows, /function New-ArcaneInstallAcl\(\[bool\]\$container\)/);
assert.match(windows, /SecurityIdentifier\('S-1-5-32-545'\)/);
assert.match(windows, /FileSystemRights\]::ReadAndExecute/);
assert.match(windows, /Arcane installation ACL is missing the required rule/);
...
for (const build of [windowsBuild, targetBuild]) {
  ...
  assert.match(build, /Import-Module .*windows-signing\.psm1/);
  assert.match(build, /Resolve-ArcaneWindowsSigningContext -RequireSigned \$requireSignedRelease/);
  assert.match(build, /Invoke-ArcaneAuthenticodeSign -Context \$signing -Path/);
  assert.match(build, /AssemblyMetadata\("ArcanePublisherBinding", "\$publisherBinding"\)/);
}
assert.match(windowsSigning, /signtool\.exe/);
assert.match(windowsSigning, /'\/fd', 'SHA256', '\/tr', \[string\]\$Context\.TimestampServer/);
assert.match(windowsSigning, /'\/td', 'SHA256'/);
assert.doesNotMatch(windowsSigning, /Set-AuthenticodeSignature/);
assert.match(windowsSigning, /Cert:\\CurrentUser\\My,Cert:\\LocalMachine\\My -CodeSigningCert/);
assert.match(windowsSigning, /HasPrivateKey/);
assert.match(windowsSigning, /CertificateMachineStore/);
assert.match(windowsSigning, /TimeStamperCertificate/);
assert.match(windowsSigning, /ARCANE_EXPECTED_PUBLISHER_THUMBPRINT/);
assert.match(windowsSigning, /ARCANE-PUBLISHER\|1\|UNSIGNED-LOCAL-TEST/);
