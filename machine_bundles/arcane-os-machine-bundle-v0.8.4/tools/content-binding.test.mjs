...
test('Windows builders bind only finalized content and require timestamps for every signed build', async () => {
  ...
  assert.match(target, /Import-Module .*windows-signing\.psm1/);
  assert.match(host, /Import-Module .*windows-signing\.psm1/);
  assert.match(signing, /ARCANE_TIMESTAMP_SERVER is required whenever an Arcane signing certificate is configured/);
