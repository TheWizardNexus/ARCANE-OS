...
for (const contract of [
  ...
  assert.match(script, new RegExp(contract.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
    `Windows iteration build contract is missing: ${contract}`);
}

assert.doesNotMatch(script, /build-app[.]mjs|build-windows-target-app[.]ps1|['"]--all['"]|\s--all(?:\s|$)/m,
  'The fast Windows iteration path must not rebuild target applications.');
assert.doesNotMatch(script, /Join-Path \$distRoot 'windows'(?!-iteration)/,
  'The fast Windows iteration path must not publish over the canonical Windows distribution.');
...
assert.match(packageManifest.scripts['build:distribution:windows'], /build-windows-release[.]ps1/,
  'Production Windows builds must continue to use the complete release builder.');
