...
for (const platform of ['windows', 'linux']) {
  ...
  try {
    ...
    if (platform === 'windows') {
      ...
      assert(payload.integrity.files.some((entry) => entry.installPath === 'bin/ArcaneOllamaService.exe'));
      assert(payload.files.some((entry) => entry.installPath === 'bin/ArcaneOllamaService.exe'));
