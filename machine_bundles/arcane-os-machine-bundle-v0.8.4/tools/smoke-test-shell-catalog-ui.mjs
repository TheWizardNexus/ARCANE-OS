...
assert.match(shell,/id="settings"/,'the default shell must expose user settings');
assert.match(shell,/Arcane\.ollama\.select\(preference\)/,'model settings must use the capability-gated Arcane API');
assert.match(shell,/option value="120b"/,'model settings must expose Arcane 120B');
...
const initializeStart=shell.indexOf('async function initialize()');
const initializeEnd=shell.indexOf('function setSessionActionPending',initializeStart);
assert(initializeStart>0&&initializeEnd>initializeStart);
assert.doesNotMatch(shell.slice(initializeStart,initializeEnd),/Arcane[.]ollama[.]selection\(\)/,'Shell startup must not repeat the on-demand GPU and model-selection probe');
...
function shellHarness(){
  const ids=['toast','errorTitle','errorMessage','errorResolution','errorBody','errorPanel','status','dismissError','copyError','clock','securityWarning','catalogState','catalogBadge','appGrid','version','identity','userValue','osValue','hostValue','settings','settingsDialog','modelPreference','modelSelectionSummary','cancelSettings','saveSettings','lock','logout','logoutDialog','cancelLogout','confirmLogout','logoutText'];
  ...
  const Arcane={
    ...
    ollama:{selection:async()=>{selectionCalls.push(true);return {preference:'auto',recommendedVariant:'20b',effectiveVariant:'20b',gpu:{devices:[{name:'Test GPU'}],largestMemoryBytes:16e9}};},select:async()=>({variant:'20b'})},
    applications:{
...
assert.equal(behavior.selectionCalls.length,0,'Shell startup must not probe GPU/model selection before Settings is opened');
behavior.document.elements.get('settings').listeners.get('click')();
await flushTasks();
assert.equal(behavior.selectionCalls.length,1,'opening Settings must perform exactly one on-demand model-selection probe');
