...
async function buildFixture(name, options = {}) {
  ...
  await writeJson(root, 'app/provisioner/manifest.webmanifest', { name: 'Arcane Provisioner' });
  await write(root, 'app/shell/index.html', '<!doctype html><title>Shell</title>\n');
  await writeJson(root, 'app/shell/manifest.webmanifest', { name: 'Arcane Shell' });
  await write(root, 'bin/ArcaneCore.exe', 'machine-core');
  await write(root, 'bin/ArcaneOllamaService.exe', 'machine-ollama-service');
...
try {
  ...
  const installedPayload = signed.adapter.installPayload(signedFixture.root);
  assert.equal(installedPayload.selfHosted, true);
  assert.equal(installedPayload.releaseReady, true, installedPayload.releaseProblem || installedPayload.description);
  assert.equal(installedPayload.mode, 'windows-webview2');
