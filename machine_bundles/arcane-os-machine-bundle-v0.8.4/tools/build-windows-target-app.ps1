...
if (-not (Test-Path -LiteralPath $hostManifest -PathType Leaf)) { throw "The Arcane host DPI manifest is missing: $hostManifest" }
...
function Sign-ArcaneFile([string]$fileName) {
  ...
  Invoke-ArcaneAuthenticodeSign -Context $signing -Path $target
}
...
"@
  ...
  $arguments = @(
    ...
    '/define:ARCANE_TARGET_APP',"/win32icon:$icon","/win32manifest:$hostManifest","/out:$(Join-Path $stage $launcherExe)",
  ...
  & node (Join-Path $root 'tools\verify-windows-dpi.mjs') (Join-Path $stage $launcherExe)
  if ($LASTEXITCODE -ne 0) { throw 'Target host DPI manifest verification failed.' }
