# Arcane OS Machine Bundle 0.8.4
...
await Arcane.appearance.current();
await Arcane.appearance.apply({ scheme: "light" });
...
`Arcane.storage` is per-user and per-application. It accepts JSON-compatible values, writes them atomically to `<state-root>/Arcane OS/apps/<application-id>/storage.json`, limits one value to 128 KiB, and limits an application's total storage to 1 MiB. `Arcane.preferences` uses the sibling `preferences.json` file and independent capabilities. The canonical app ID is validated before either path is resolved; unowned legacy global files are preserved rather than copied into an app. Microsoft NT places the corresponding WebView2 profile at `%LOCALAPPDATA%/Arcane OS/apps/<application-id>/webview2/`; Linux gives WebKitGTK an app-specific network-session data/cache directory under the XDG roots; Android binds the WebView to the provider-managed `arcane-app-<application-id>` profile and refuses a shared-profile fallback. Browser OPFS state is nested below `apps/<application-id>/`. See the repository's `docs/application-data-isolation.md` for the full contract and migration rules.
...
```powershell
$env:ARCANE_SIGNING_CERT_THUMBPRINT = '<certificate thumbprint>'
$env:ARCANE_TIMESTAMP_SERVER = 'https://<certificate-provider-rfc3161-endpoint>'
npm run signing:preflight:windows
npm run build:signed:windows
```

The signed wrapper sets the matching `ARCANE_EXPECTED_PUBLISHER_THUMBPRINT` for the child build when it is not already configured. An explicitly configured mismatch fails closed. `ARCANE_SIGNTOOL_PATH` is optional when `signtool.exe` is discoverable from `PATH` or the Microsoft Windows SDK. The same shared signer supports narrower package builds:

```powershell
npm run build:signed:apps:windows
npm run build:signed:app:windows -- -AppId boss
```
...
`arcane-apps.json` is the registry for generic non-privileged Arcane applications. Each entry defines the app id, display name, source directory, HTML entry point, capability allowlist, security-origin policy, and explicit payload allowlist. An entry may add `bundledApps` to package another registered app's authoritative positive-allowlist snapshot under its own root. Bundles must be acyclic, cannot include the current app or an app with a generated document catalog, and cannot require capabilities or network/media/frame origins absent from the outer app. An app with `web.embed` and an empty `frameOrigins` list may frame only same-origin packaged content.
