...
The Shell's compiled startup backdrop is the sole pre-verification presentation. It is a noninteractive native solid-color surface with no file, registry, WebView, Core, or application input. Publisher verification still gates all executable release content and the first-boot pipeline.
