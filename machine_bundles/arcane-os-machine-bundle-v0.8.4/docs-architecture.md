...
`src/api/method-contracts.json` is a separate canonical semantic registry. Its first vertical slice versions seven Core/Android methods and records their effect, current audit state, direct-network behavior, operating-system permissions, policy hooks, cancellation, reversibility, idempotency, data movement, and closed exact input/output shape kinds. It cannot redefine capability, application, host, privilege, or mutation authority. Generation embeds a deeply frozen contract table into Core and a separate Android semantic-constant registry. Core enforces authorization, then exact input validation, then dispatch, then exact output and cross-field validation before any success frame. Capability-free `system.ping` returns only `{ok:true}` after trusted session/origin, method-admission, and replay checks; it performs no provider work and makes no health, readiness, privilege, or trust claim. Android consumes the generated URI, network, platform, application, list, and renderer bounds; constructs the fixed Android platform variant; rejects invalid provider status rather than truncating it; and reuses one strict mailto canonicalizer at the bridge and system adapter. Validation requires exact cross-host policy coverage and fails closed on unknown fields, authority injection, noncanonical serialization, broadened URI schemes, or changed status meaning. Shared fixtures cover empty parameters and the intentionally narrow printable-ASCII mailto grammar, including encoded controls, malformed escapes, fragments, backslashes, Unicode, and alternate schemes. `network.status.online` means only that a non-loopback interface was observed; it is not Internet or service reachability. `external.open.opened` means only that the operating system accepted the handoff; it does not prove that a composer opened or a message was sent, and simulation returns `EXTERNAL_OPEN_SIMULATED` rather than fabricating success. The current `audit: none` values are an explicit gap, not a claim that audit is unnecessary. Kotlin source parity is verified, but executable Kotlin/JVM, instrumentation, and device conformance remain pending the dependency-verified Android build.
...
## Microsoft NT account staging and shell recovery

Real account provisioning is Microsoft NT-only in 0.8.4. Linux deliberately reports user provisioning unavailable rather than replacing a POSIX login shell with a graphical executable before a display-manager-safe Arcane session exists.
...
1. `users.add` writes a prepared recovery record, creates the standard Microsoft NT account disabled, records its exact SID, initializes its profile, assigns Arcane as its shell, and writes `activation-pending` state.
