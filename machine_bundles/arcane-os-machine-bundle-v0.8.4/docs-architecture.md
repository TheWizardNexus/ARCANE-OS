...
`Arcane.filesystem.selectDirectory()` is the narrow exception for user-chosen local paths. The `filesystem.directory.select` capability opens the native operating-system folder dialog only after a user action and returns either one canonical existing directory or an explicit cancellation. It does not enumerate drives, expose directory contents, create folders, or grant storage access. Applications remain responsible for applying their own domain validation to the selected directory.
