...
- Keeps a **Verify and restore previous shell** action available when a signed-out profile is visible only through the protected recovery journal; administrator approval reloads the profile and rechecks both exact Windows shell bindings before restoring either one.
