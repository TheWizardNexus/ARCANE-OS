...
| [Arcane mail gateway](./docs/mail-gateway.md) | Shared local/hosted HTTP-to-SMTP contract, authenticated deployment topology, configuration, and operational limits. |
...
PreCrisis and Warrior Spirit automatically use the shared loopback mail gateway at port 8025 during local HTTP development. Mail remains disabled until an operator supplies SMTP values and starts `npm run mail:start`; follow [the mail gateway guide](./docs/mail-gateway.md) and never put SMTP or production app credentials in browser code.
