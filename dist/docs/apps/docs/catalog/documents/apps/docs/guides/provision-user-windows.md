# Provision and sign in your first Arcane OS user on Windows

Arcane OS 0.8.4 supports the complete Windows first-user journey. An authorized administrator uses the trusted native **Arcane Provisioner**; the new user later authenticates through Windows and enters Arcane Shell without a second Arcane login.

> The screenshots below are controlled Windows simulations. They show the current labels and state transitions, but no real account was changed, no credential is visible, and the images do not establish clean-host acceptance. Never put a temporary password in a screenshot, operation log, chat, issue, source file, or automated clipboard stream.
...
2. If necessary, use the Provisioner's install, update, or repair action before adding a user. Approve a Windows elevation prompt only when its application, publisher, action, and target match what the Provisioner just described.
...
![Controlled Windows simulation of Add Arcane users with a validated example username and the Add Arcane user button.](../../../../../screenshots/windows-add-arcane-user.jpg)
...
![Controlled Windows simulation of a staged disabled account awaiting activation; no temporary password is shown.](../../../../../screenshots/windows-account-awaiting-activation.jpg)

6. Choose **Activate this account** on that user card. Activation is a separate privileged request and may cause a separate Windows elevation prompt.
7. Confirm that the same account is now marked **Arcane Shell assigned** and **Enabled**.

![Controlled Windows simulation after activation, showing the account as Arcane Shell assigned and enabled.](../../../../../screenshots/windows-account-activated.jpg)

## First sign-in

8. Sign out of the administrator session. At the Windows sign-in screen, choose the new local account and enter the temporary password privately.
9. Change the password when Windows requires it. Do not automate, record, or screenshot the password-entry or password-change screens.
10. After Windows accepts the sign-in, the verified Arcane Shell starts as the user's shell. There is no Arcane username or password form. Confirm the release identity, open Settings, and verify that logout and the documented recovery path are reachable.

![Controlled Windows simulation of Arcane Shell after launch, showing the release, signed-in operating-system user, and Settings entry point.](../../../../../screenshots/windows-arcane-shell.jpg)
...
1. Choose **Prepare temporary password**. This prepares and displays a credential for private review; it does not change the Windows password.
2. Save the credential privately.
3. Choose **I saved this — apply password**. Only this separate privileged request changes the Windows password and requires a change at next sign-in.
