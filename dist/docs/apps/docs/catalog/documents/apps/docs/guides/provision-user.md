...
Arcane OS 0.8.4 supports the complete first-user journey on Windows: an administrator stages a standard local account, saves its temporary credential, activates it in a separate request, and then the user signs in through Windows. Arcane Shell starts as that account's shell; there is no separate Arcane sign-in form.

[Open the Windows provisioning and sign-in walkthrough](provision-user-windows.md)
