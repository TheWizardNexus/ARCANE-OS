...
function searchTokens(value){
    return [...new Set(normalizedSearchText(value).match(/[\p{L}\p{N}]+/gu)??[])].slice(0,32);
}
...
export default class StaticDocumentCatalog{
    ...
    async buildContext(query,options={}){
        ...
        assertKnownKeys(options,new Set(['limit','maxCharacters','maxDocumentCharacters','signal']),'Context options','STATIC_DOCUMENT_INVALID_OPTIONS');
        ...
        const matches=this.search(query,{limit:Math.min(limit,this.#limits.maxResults)});
        ...
        for(const match of matches){
            ...
            let hydrated;
            try{
                hydrated=await this.hydrate(match.id,{signal:options.signal});
            }catch(error){
                if(error?.code==='STATIC_DOCUMENT_ABORTED') throw error;
            ...
            const excerpt=safeSlice(hydrated.text,allowed);
