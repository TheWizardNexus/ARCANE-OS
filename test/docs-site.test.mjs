...
test('provisioning preserves credential delivery, activation, and recovery boundaries',()=>{
    ...
    assert.match(provisionGuide,/No host logout or login is required/i);
    assert.match(provisionGuide,/start-shell\.bat/);
    assert.match(provisionGuide,/do not press \*\*Log out\*\*/i);
});
...
test('developer setup uses locked dependencies and the supported package gates',()=>{
    ...
    assert.match(developerGuide,/machine_bundles\\arcane-os-machine-bundle-v0\.8\.4\\dist\\windows\\bin\\ArcaneProvisioner\.exe/);
...
test('Linux host guide states the runnable path and the unsupported release boundary',()=>{
    ...
    assert.match(linuxGuide,/does not yet provision Linux accounts/i);
...
test('browser checks hydrate every published document and reviewed source file',()=>{
    ...
    assert.match(app,/item\.record\.id==='linux-host'/);
